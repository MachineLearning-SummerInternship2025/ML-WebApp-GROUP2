...
if st.session_state.get('authenticated', False):
    # Show navbar with Dashboard and Logout when authenticated
    st.markdown("""
    <div class="navbar" style="display: flex; justify-content: center; padding: 10px 40px;">
        <a href="/?page=about" style="background-color:#2563eb; color:white; padding:4px 12px; border-radius:6px; text-decoration:none; font-weight:500; font-size:14px; font-family:'Cocogoose-Semilight'; margin-right: 10px;">
            About
        </a>
        <a href="/?page=dashboard" style="background-color:#2563eb; color:white; padding:4px 12px; border-radius:6px; text-decoration:none; font-weight:500; font-size:14px; font-family:'Cocogoose-Semilight'; margin-right: 10px;">
            Dashboard
        </a>
        <button onclick="logout()" style="background-color:#dc2626; color:white; padding:4px 12px; border-radius:6px; border:none; font-weight:500; font-size:14px; font-family:'Cocogoose-Semilight'; cursor:pointer;">
            Logout
        </button>
    </div>
    ...
    <script>
    function logout() {
        // Send logout request to Streamlit
        window.parent.postMessage({
            type: 'streamlit:setComponentValue',
            value: 'logout_requested'
        }, '*');
    ...
    # Show navbar with About and Get Started when not authenticated
    st.markdown("""
    <div class="navbar" style="display: flex; justify-content: center; padding: 10px 40px;">
        <a href="/?page=about" style="background-color:#2563eb; color:white; padding:4px 12px; border-radius:6px; text-decoration:none; font-weight:500; font-size:14px; font-family:'Cocogoose-Semilight'; margin-right: 10px;">
            About
        </a>
        <a href="/?page=get_started" style="background-color:#2563eb; color:white; padding:4px 12px; border-radius:6px; text-decoration:none; font-weight:500; font-size:14px; font-family:'Cocogoose-Semilight';">
            Get Started
        </a>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.current_page == 'about':
        ...
        with col1:
            st.markdown(
                ...
                <div style="padding-left:40px; max-width:600px;">
                    <div style="font-size: 70px; font-family:'BukaBird'; color:#2563eb; margin-bottom: 12px;">
                        ML Explorer
                    </div>
                    <div style="font-size: 30px; font-family:'Cocogoose-Bold'; margin-bottom: 18px; color:#000000;">
                        Learn Machine Learning by Doing.
                    </div>
                    <div style="font-size: 14px; font-family:'Cocogoose-Regular'; line-height: 1.5; margin-bottom: 12px; color:#000000;">
                        Our website is an interactive platform that lets you upload datasets, 
                        explore data, and train machine learning models - all without needing coding skills.
                    </div>
                    <div style="font-size: 12px; font-family:'Cocogoose-Light'; line-height: 1.5; margin-bottom: 20px; color:#000000;">
    ...
    st.markdown(
        """
<div style="margin-top:80px; text-align:center; font-size:40px; font-family:'Cocogoose-Bold'; color:#000000; margin-bottom:40px;">
    How do I use it?
</div>
""",
        unsafe_allow_html=True,
    ...
    spacer1, col1, col2, col3, spacer2 = st.columns([1, 2, 2, 2, 1])  # keeps everything centered

    with col1:
        st.image(upload_image_path, caption=None, width=200)
        st.markdown(
            """
            <div style="text-align:center;">
                <div style="font-family:'Cocogoose-Bold'; font-size:18px; color:#000000; margin-top:10px;">
                    Upload Dataset
                </div>
                <div style="font-family:'Cocogoose-Light'; font-size:16px; color:#444444; margin-top:5px;">
    ...
    with col2:
        st.image(graph_image_path, caption=None, width=200)
        st.markdown(
            """
            <div style="text-align:center;">
                <div style="font-family:'Cocogoose-Bold'; font-size:18px; color:#000000; margin-top:10px;">
    ...
    with col3:
        st.image(train_image_path, caption=None, width=200)
        st.markdown(
            """
            <div style="text-align:center;">
                <div style="font-family:'Cocogoose-Bold'; font-size:18px; color:#000000; margin-top:10px;">
    ...
    st.markdown("Welcome! Please login or create an account to start using ML Explorer.")
    
    # Configuration info for non-authenticated users
    with st.expander("⚙️ Configuration & État", expanded=False):
        if SECURITY_AVAILABLE:
        ...
        if CLOUD_AVAILABLE:
                ...
                if dropbox:
                    try:
                        ...
                        if storage_info.get("storage_type") == "dropbox":
                            st.success(f"☁️ Dropbox: {storage_info.get('total_files', 0)} fichiers")
                        else:
            ...
            st.info("💾 Stockage local uniquement")
...
if st.session_state.authenticated:
    ...
    if st.session_state.current_page == 'dashboard':
        ...
        st.markdown("Welcome to your ML Explorer dashboard! Use the sidebar to navigate through different features.")
...
# Initialize variables at higher scope
df = None
dataset_id = None
uploaded_file = None
        ...
        if automl_ready:
            ...
            if run:
                try:
                    ...
                    pdf_path = generate_pdf_report_reportlab(
                        df, model_choice, params, target_column, results, all_figures
                    )
                    with open(pdf_path, "rb") as f:
                        st.download_button(
                            "📥 Download PDF Report",
                            data=f.read(),
                            file_name=f"ml_report_{st.session_state.username}.pdf",
                    ...
                    st.session_state["trained_model_features"] = [
                        col for col in X_train.columns if col != target_column
                    ]  # Exclude target
                    ...
                    st.session_state["preprocess_func"] = lambda df: advanced_preprocessing_pipeline(
                        ...
                        encode_method="ordinal",
                        drop_useless=False,
                        encoder=st.session_state.get("feature_encoder"),
                        fit_encoder=False
                    )[[col for col in X_train.columns if col != target_column]]  # Exclude target
                    st.success(
                        "✅ Model trained and saved! You can now go to the Prediction tab to make predictions on new data."
                    )
...
# Close the sidebar authentication check
